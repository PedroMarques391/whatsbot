 ...
 */
export async function isNotInGroup(
  chat: GroupChat,
  message: Message,
): Promise<boolean> {
  if (
    ...
    throw new Error(
      `Acho que você se confundiu. O número "${extractTextFromBody(message.body)}" não consta na nossa lista de participantes deste grupo.`,
    );
  }
  return false;
}
